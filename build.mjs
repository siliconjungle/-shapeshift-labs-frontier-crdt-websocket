...
function linkLocalPackage(name, targetDir) {
  ...
  const targetLink = path.relative(path.dirname(linkPath), targetDir);
  fs.mkdirSync(scopeDir, { recursive: true });
  try {
    const stat = fs.lstatSync(linkPath);
    if (!stat.isSymbolicLink()) return;
    if (fs.readlinkSync(linkPath) === targetLink) return;
    fs.unlinkSync(linkPath);
  } catch (error) {
    if (error.code !== 'ENOENT') throw error;
  }
  try {
    fs.symlinkSync(targetLink, linkPath, 'dir');
  } catch (error) {
    if (error.code !== 'EEXIST') throw error;
    const stat = fs.lstatSync(linkPath);
    if (!stat.isSymbolicLink() || fs.readlinkSync(linkPath) !== targetLink) throw error;
  }
}
